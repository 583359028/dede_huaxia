...
class JSSDK {
    ...
    public function get_sysset(){
        ...
        return pdo_fetch("SELECT * FROM ".tablename('amouse_weicard_sysset')." WHERE weid= :uniacid limit 1", array(':uniacid' => $this->uniacid));
    ...
    private function getJsApiTicket() {
        ...
        $api = cache_load("zombie.ecard.api_share.json::".$this->uniacid, true);
        ...
        $data = cache_load("zombie.ecard.jsapi_ticket.json::".$this->uniacid, true);
        ...
        if (empty($data['expire_time']) || $data['expire_time'] < time() || $new) {
            ...
            if ($ticket) {
                ...
                cache_write("zombie.ecard.jsapi_ticket.json::".$this->uniacid, iserializer($data));

                cache_write("zombie.ecard.api_share.json::".$this->uniacid, iserializer(array("appid"=>$this->appId,"appsecret"=>$this->appSecret)));
    ...
    private function getAccessToken() {
        ...
        $data = cache_load("zombie.ecard.access_token.json::".$this->uniacid, true);
        ...
        if (empty($data['expire_time']) || $data['expire_time'] < time() || $new) {
            ...
            if ($access_token) {
                ...
                cache_write("zombie.ecard.access_token.json::".$this->uniacid, iserializer($data));
