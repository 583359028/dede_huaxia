...
class Pay_EweiShopV2Page extends AppMobilePage
{
	public function main()
	{
		...
		app_json(array(
	'order'  => array('id' => $order['id'], 'ordersn' => $order['ordersn'], 'price' => $order['price']),
	'credit' => $credit,
	'alipay' => $alipay,
	'wechat' => $wechat
	));
	...
	protected function success($orderid)
	{
		...
		app_json(array(
	'order'   => array('id' => $orderid, 'isverify' => $order['isverify'], 'virtual' => $order['virtual'], 'isvirtual' => $order['isvirtual'], 'isvirtualsend' => $order['isvirtualsend'], 'status' => $order['paytype'] == 3 ? '订单提交支付' : '订单支付成功', 'text' => $text),
