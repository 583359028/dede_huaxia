...
class MemberLogin
{
    ...
    function App_GetEncodePwd($pwd)
    {
        ...
        {
            case 'l16':
                return substr($pwd, 0, 16);
            case 'r16':
                return substr($pwd, 16, 16);
            case 'm16':
...
///app token
    function App_CheckUser(&$loginuser, $loginpwd)
    {
        global $dsql;
        ...
        if(is_array($row))
        {
            if($this->GetShortPwd($row['pwd']) != $this->App_GetEncodePwd($loginpwd))
            {
                return -1;
            }
            else
