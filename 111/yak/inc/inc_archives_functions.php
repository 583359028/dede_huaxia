 ...
 *
 * @access    public
 * @param     string  $litpic  视频
 * @param     string  $picname  视频名称
 * @param     string  $isremote  是否远程
 * @return    string
 */
function GetDDVideo($litpic, $picname, $isremote)
{
    global $cuserLogin,$cfg_ddimg_width,$cfg_ddimg_height,$cfg_basedir,$ddcfg_image_dir,$cfg_addon_savetype;
    $ntime = time();
    if( ($litpic != 'none' || $litpic != 'ddfirst') && 
     !empty($_FILES[$litpic]['tmp_name']) && is_uploaded_file($_FILES[$litpic]['tmp_name']))
    {
        //如果用户自行上传缩略图
        $istype = 0;
        $sparr = Array("video/mp4", "video/avi", "video/rmvb", "video/wmv");
        $_FILES[$litpic]['type'] = strtolower(trim($_FILES[$litpic]['type']));
        if(!in_array($_FILES[$litpic]['type'], $sparr))
        {
            ShowMsg("上传的视频格式错误，请使用MP4、avi、rmvb\wmv格式的其中一种！","-1");
            exit();
        }
        $savepath = $ddcfg_image_dir.'/'.MyDate($cfg_addon_savetype, $ntime);
        ...
        CreateDir($savepath);
        $fullUrl = $savepath.'/'.dd2char(MyDate('mdHis', $ntime).$cuserLogin->getUserID().mt_rand(1000, 9999));
        if(strtolower($_FILES[$litpic]['type']) == "video/rmvb")
        {
            $fullUrl = $fullUrl.".rmvb";
        }
        else if(strtolower($_FILES[$litpic]['type']) == "video/avi")
        {
            $fullUrl = $fullUrl.".avi";
        }
        else
